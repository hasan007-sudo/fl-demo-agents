"""Base class for SpeakWithAI multi-agent system."""
...
class BaseSpeakWithAIAgent(TimingMixin, BaseAgent[SpeakWithAIContext]):
    """
    Base class for SpeakWithAI agents with shared handoff functionality.
    ...
    def __init__(
        self,
        instructions: str,
        context: Optional[SpeakWithAIContext] = None,
        chat_ctx: Optional[ChatContext] = None,
        **kwargs
    ):
        """
        Initialize the base SpeakWithAI agent.

        Args:
            instructions: System prompt for this agent
            context: SpeakWithAI context from frontend
    ...
    async def _validate_context_hook(self, context: SpeakWithAIContext) -> bool:
        """Validate the SpeakWithAI context."""
    ...
    async def _on_session_timeout(self, checkpoint: Checkpoint, idx: int) -> None:
        ...
        For ConversationAgent: triggers transfer_to_feedback()
        """
        agent_name = self.__class__.__name__
